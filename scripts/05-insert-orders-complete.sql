...
-- Test kullanıcısının var olduğundan emin ol
...
((SELECT id FROM orders WHERE order_number = 'ORD-2024-002'), 'processing', 'Ödeme onaylandı', 1, NOW() - INTERVAL '4 hours'),
...
((SELECT id FROM orders WHERE order_number = 'ORD-2024-005'), 'cancelled', 'Müşteri talebi ile iptal', 1, NOW() - INTERVAL '3 days'),

((SELECT id FROM orders WHERE order_number = 'ORD-2024-006'), 'pending', 'Sipariş alındı', 1, NOW() - INTERVAL '6 hours'),
((SELECT id FROM orders WHERE order_number = 'ORD-2024-006'), 'processing', 'Ödeme onaylandı', 1, NOW() - INTERVAL '2 hours');
